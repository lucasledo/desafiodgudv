...
@endif
        ...
        <td>
            ...
            <button class="btn updateStatus" action="{{ route('tasks.update', $task->id) }}" value="{{$task->status == 1 ? 'true' : 'false'}}" title="{{$task->status == 0 ? __('Set Done Task') : __('Unset Done Task')}}">
